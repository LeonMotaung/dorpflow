<?php
/**
 * DorpFlow ERP - Resident Citizen Public Registration Form
 */
?>
<style>
...
<div class="container d-flex align-items-center justify-content-center" style="min-height: 100vh; padding-top: 100px; padding-bottom: 40px;">
    <div class="register-card overflow-hidden">
        ...
        <div class="text-center py-4 border-bottom bg-white">
            ...
            <p class="mb-0 text-muted mt-2" style="font-size:0.75rem; font-weight:700; letter-spacing:0.05em;">CITIZEN PORTAL REGISTRATION</p>
        </div>
        ...
        <div class="card-body p-4">
            ...
            <form action="<?php echo APP_URL; ?>/public/index.php/register" method="POST">
                <input type="hidden" name="csrf_token" value="<?php echo $csrf_token; ?>">
                
                <div class="mb-3">
                    <label for="regName" class="form-label">Full Name *</label>
                    <input type="text" name="full_name" class="form-control" id="regName" placeholder="e.g. Sarah Miller" required>
                </div>

                <div class="mb-3">
                    <label for="regEmail" class="form-label">Email Address *</label>
                    <input type="email" name="email" class="form-control" id="regEmail" placeholder="e.g. sarah.m@gmail.com" required>
                </div>

                <div class="mb-3">
                    <label for="regPhone" class="form-label">Mobile Phone Number *</label>
                    <input type="tel" name="phone" class="form-control" id="regPhone" placeholder="e.g. 0715559876" required>
                </div>

                <div class="mb-3">
                    <label for="regWard" class="form-label">Municipal Ward *</label>
                    <select name="ward_id" class="form-select" id="regWard" required>
                        <option value="" disabled selected>Select your Ward</option>
                        <?php foreach ($wards as $w): ?>
                            <option value="<?php echo $w['id']; ?>">Ward <?php echo $w['ward_number']; ?> (Cllr: <?php echo $w['councilor_name']; ?>)</option>
                        <?php endforeach; ?>
                    </select>
                </div>

                <div class="mb-3">
                    <label for="regPass" class="form-label">Choose Password *</label>
                    <input type="password" name="password" class="form-control" id="regPass" placeholder="••••••••" required>
                </div>

                <button type="submit" class="btn btn-custom btn-primary-custom w-100 py-3 mb-2">Register Account</button>
            </form>
            
            <div class="text-center mt-3" style="font-size:0.8rem; color:#64748B;">
                Already have an account? <a href="<?php echo APP_URL; ?>/public/index.php/login" class="text-primary text-decoration-none fw-semibold">Sign In here</a>.
            </div>
        </div>
    </div>
</div>
